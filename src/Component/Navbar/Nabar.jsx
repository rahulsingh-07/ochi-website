...
export default function Nabar() {
  ...
  return (
    <div className='flex justify-between backdrop-filter backdrop-blur-lg
    px-6 py-4 items-baseline z-50 fixed w-full '>
      <div>
      ...
      <div>
       <ul className='md:flex hidden gap-3' >
        {Data.map((item)=>(
            <li key={item.id}
            className={` text-md capitalize  ${item.id===5 &&"ml-36"}`}          
            >
<a className='relative group'
 href="{item.link}">{item.title} <span className='absolute rounded-full w-full h-[1.5px] bottom-0 left-0 transform
 scale-x-0 group-hover:scale-x-100 transition-transform group-hover:duration-500 duration-500 origin-left bg-black '></span></a>
           </li>
        ))}
       </ul>
      </div>
